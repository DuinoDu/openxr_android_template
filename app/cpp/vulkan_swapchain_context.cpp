...
void VulkanSwapchainContext::InitSwapchainImageViews() {
  ...
  for (size_t i = 0; i < swapchain_images_.size(); i++) {
    rendering_context_->CreateImageView(swapchain_images_[i].image, swapchain_image_format_, VK_IMAGE_ASPECT_COLOR_BIT,
...
void VulkanSwapchainContext::Draw(uint32_t image_index, std::shared_ptr<vulkan::VulkanRenderingPipeline> pipeline,
                                  uint32_t index_count, std::vector<glm::mat4> transforms) {
  ...
  for (const auto &transform : transforms) {
    vkCmdPushConstants(graphics_command_buffers_[current_fame_], pipeline->GetPipelineLayout(),
  ...
  if (vkQueueSubmit(rendering_context_->GetGraphicsQueue(), 1, &submit_info, in_flight_fences_[current_fame_]) !=
      VK_SUCCESS) {
...
[[nodiscard]] bool VulkanSwapchainContext::IsInited() const { return inited_; }
...
VulkanSwapchainContext::~VulkanSwapchainContext() {
  ...
  vkFreeCommandBuffers(rendering_context_->GetDevice(), rendering_context_->GetGraphicsPool(),
                       graphics_command_buffers_.size(), &graphics_command_buffers_[0]);
...
void VulkanSwapchainContext::CreateColorResources() {
  rendering_context_->CreateImage(swapchain_extent_.width, swapchain_extent_.height,
                                  rendering_context_->GetRecommendedMsaaSamples(), swapchain_image_format_,
                                  VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &color_image_, &color_image_memory_);
  rendering_context_->CreateImageView(color_image_, swapchain_image_format_, VK_IMAGE_ASPECT_COLOR_BIT,
                                      &color_image_view_);
  rendering_context_->TransitionImageLayout(color_image_, VK_IMAGE_LAYOUT_UNDEFINED,
...
void VulkanSwapchainContext::CreateDepthResources() {
  ...
  rendering_context_->CreateImage(swapchain_extent_.width, swapchain_extent_.height,
                                  rendering_context_->GetRecommendedMsaaSamples(), depth_format,
                                  VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                  &depth_image_, &depth_image_memory_);
  rendering_context_->CreateImageView(depth_image_, depth_format, VK_IMAGE_ASPECT_DEPTH_BIT, &depth_image_view_);
  rendering_context_->TransitionImageLayout(depth_image_, VK_IMAGE_LAYOUT_UNDEFINED,
                                            VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
}

void VulkanSwapchainContext::CreateFrameBuffers() {
  for (size_t i = 0; i < swapchain_image_views_.size(); i++) {
    std::array<VkImageView, 3> attachments = {color_image_view_, depth_image_view_, swapchain_image_views_[i]};
    ...
    CHECK_VKCMD(
        vkCreateFramebuffer(rendering_context_->GetDevice(), &framebuffer_info, nullptr, &swapchain_frame_buffers_[i]));
  }
}
...
void VulkanSwapchainContext::CreateCommandBuffers() {
  ...
  CHECK_VKCMD(vkAllocateCommandBuffers(rendering_context_->GetDevice(), &alloc_info, graphics_command_buffers_.data()));
}
