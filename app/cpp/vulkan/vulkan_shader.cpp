...
vulkan::VulkanShader::VulkanShader(const std::shared_ptr<VulkanRenderingContext> &context,
                                   const std::vector<uint32_t> &code, std::string entry_point_name)
    : code_(std::move(code)), entry_point_name_(std::move(entry_point_name)), device_(context->GetDevice()) {
  ...
  SpvReflectResult result =
      spvReflectCreateShaderModule(code_.size() * sizeof(uint32_t), code_.data(), &reflect_shader_module_);
  ...
  uint32_t count = 0;
  result = spvReflectEnumerateEntryPointPushConstantBlocks(&reflect_shader_module_, this->entry_point_name_.data(),
                                                           &count, nullptr);
...
const std::vector<VkPushConstantRange> &vulkan::VulkanShader::GetPushConstants() const { return push_constants_; }
