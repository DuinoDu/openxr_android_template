...
vulkan::VulkanRenderingPipeline::VulkanRenderingPipeline(std::shared_ptr<VulkanRenderingContext> context,
                                                         std::shared_ptr<VulkanShader> vertex_shader,
                                                         std::shared_ptr<VulkanShader> fragment_shader,
                                                         const VertexBufferLayout &vbl, RenderingPipelineConfig config)
    : context_(context), device_(context_->GetDevice()), config_(config) {
...
void vulkan::VulkanRenderingPipeline::SetIndexBuffer(std::shared_ptr<VulkanBuffer> buffer, DataType element_type) {
...
void vulkan::VulkanRenderingPipeline::CreatePipeline(const VertexBufferLayout &vbl) {
  VkPipelineShaderStageCreateInfo shader_stages[] = {vertex_shader_->GetShaderStageInfo(),
  ...
  pipeline_push_constants.insert(pipeline_push_constants.end(), fragment_push_constants.begin(),
  ...
  CHECK_VKCMD(vkCreateGraphicsPipelines(device_, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline_));
}
...
VkPipelineLayout vulkan::VulkanRenderingPipeline::GetPipelineLayout() const { return pipeline_layout_; }
