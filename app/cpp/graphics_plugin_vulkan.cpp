...
namespace {
const std::vector<float> kCubePositions = {-0.5, -0.5, 0.5,  1.0, 0.0, 0.0, 0.5,  -0.5, 0.5,  0.0, 1.0, 0.0,
                                           0.5,  0.5,  0.5,  0.0, 0.0, 1.0, -0.5, 0.5,  0.5,  1.0, 1.0, 1.0,
                                           -0.5, -0.5, -0.5, 1.0, 0.0, 0.0, 0.5,  -0.5, -0.5, 0.0, 1.0, 0.0,
                                           0.5,  0.5,  -0.5, 0.0, 0.0, 1.0, -0.5, 0.5,  -0.5, 1.0, 1.0, 1.0};
const std::vector<unsigned short> kCubeIndices{0, 1, 2, 2, 3, 0, 1, 5, 6, 6, 2, 1, 7, 6, 5, 5, 4, 7,
...
class VulkanGraphicsPlugin : public GraphicsPlugin {
  ...
  void InitializeDevice(XrInstance xr_instance, XrSystemId system_id) override {
    ...
    CHECK_XRCMD(pfn_get_vulkan_graphics_requirements_khr(xr_instance, system_id, &graphics_requirements));
    ...
    instance_create_info.ppEnabledExtensionNames = instance_extensions.empty() ? nullptr : instance_extensions.data();
    ...
    CHECK_XRCMD(pfn_xr_create_vulkan_instance_khr(xr_instance, &vulkan_instance_create_info_khr, &vulkan_instance_,
    ...
    if (debug_utils_enabled) {
      ...
      CHECK_VKCMD(CreateDebugUtilsMessengerExt(vulkan_instance_, &create_info, nullptr, &debug_messenger_));
    }
    ...
    CHECK_XRCMD(
        pfn_get_vulkan_graphics_device_khr(xr_instance, &vulkan_graphics_device_get_info_khr, &physical_device_));
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(physical_device_, &queue_family_count, &queue_family_properties[0]);
    ...
    CHECK_XRCMD(pfn_xr_create_vulkan_device_khr(xr_instance, &vulkan_device_create_info_khr, &logical_device_,
...
#include "frag.spv"
    ...
    auto vertex_shader = std::make_shared<vulkan::VulkanShader>(rendering_context_, kVertexShader, "main");
    auto fragment_shader = std::make_shared<vulkan::VulkanShader>(rendering_context_, kFragmentShader, "main");
    ...
    pipeline_ = std::make_shared<vulkan::VulkanRenderingPipeline>(rendering_context_, vertex_shader, fragment_shader,
                                                                  vertex_buffer_layout, pipeline_config);
    auto vertex_buffer =
        std::make_shared<vulkan::VulkanBuffer>(rendering_context_, sizeof(float) * kCubePositions.size(),
                                               VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    vertex_buffer->Update(kCubePositions.data());
    pipeline_->SetVertexBuffer(vertex_buffer);

    auto index_buffer =
        std::make_shared<vulkan::VulkanBuffer>(rendering_context_, sizeof(unsigned short) * kCubeIndices.size(),
  ...
  [[nodiscard]] int64_t SelectSwapchainFormat(const std::vector<int64_t> &runtime_formats) override {
    ...
    constexpr VkFormat kPreferredSwapchainFormats[] = {
        // 4 channel formats
        VK_FORMAT_R8G8B8A8_SRGB, VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_B8G8R8A8_SRGB, VK_FORMAT_B8G8R8A8_UNORM};

    auto swapchain_format_it =
        std::find_first_of(runtime_formats.begin(), runtime_formats.end(), std::begin(kPreferredSwapchainFormats),
    ...
    rendering_context_ = std::make_shared<vulkan::VulkanRenderingContext>(
        physical_device_, logical_device_, graphic_queue_, graphics_command_pool_, (VkFormat)(*swapchain_format_it));
    InitializeResources();
  ...
  XrSwapchainImageBaseHeader *AllocateSwapchainImageStructs(
    ...
    auto swapchain_context =
        std::make_shared<VulkanSwapchainContext>(rendering_context_, capacity, swapchain_create_info);
  ...
  void RenderView(const XrCompositionLayerProjectionView &layer_view, XrSwapchainImageBaseHeader *swapchain_images,
                  const uint32_t image_index, const std::vector<math::Transform> &cube_transforms) override {
    ...
    for (const math::Transform &cube : cube_transforms) {
      glm::mat4 model = glm::scale(
          glm::translate(glm::identity<glm::mat4>(), cube.position) * glm::mat4_cast(cube.orientation), cube.scale);
      transforms.emplace_back(proj * view * model);
    }
    auto swapchain_context = image_to_context_mapping_[swapchain_images];

    swapchain_context->Draw(image_index, pipeline_, kCubeIndices.size(), transforms);
  }
 ...
 private:
  ...
  std::map<XrSwapchainImageBaseHeader *, std::shared_ptr<VulkanSwapchainContext>> image_to_context_mapping_{};
};
}  // namespace

std::shared_ptr<GraphicsPlugin> CreateGraphicsPlugin() { return std::make_shared<VulkanGraphicsPlugin>(); }
