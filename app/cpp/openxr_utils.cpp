...
void LogLayersAndExtensions() {
  const auto log_extensions = [](const char *layer_name, int indent = 0) {
    ...
    CHECK_XRCMD(xrEnumerateInstanceExtensionProperties(layer_name, extensions.size(), &instance_extension_count,
                                                       extensions.data()));

    const std::string kIndentStr(indent, ' ');
    spdlog::debug("{} Available Extensions: ({})", kIndentStr.c_str(), instance_extension_count);
    for (const XrExtensionProperties &extension : extensions) {
      spdlog::debug("{}  Name={} SpecVersion={}", kIndentStr.c_str(), extension.extensionName,
...

  {
    ...
    for (const XrApiLayerProperties &layer : layers) {
      spdlog::debug("  Name={} SpecVersion={} LayerVersion={} Description={}", layer.layerName,
...
void LogInstanceInfo(XrInstance instance) {
  ...
  spdlog::info("Instance RuntimeName={} RuntimeVersion={}", instance_properties.runtimeName,
...
void LogViewConfigurations(XrInstance instance, XrSystemId system_id) {
  ...
  CHECK_XRCMD(xrEnumerateViewConfigurations(instance, system_id, view_config_type_count, &view_config_type_count,
  ...
  for (XrViewConfigurationType view_config_type : view_config_types) {
    ...
    if (view_count > 0) {
      ...
      CHECK_XRCMD(xrEnumerateViewConfigurationViews(instance, system_id, view_config_type, view_count, &view_count,
                                                    views.data()));

      for (uint32_t i = 0; i < views.size(); i++) {
        const XrViewConfigurationView &view = views[i];

        spdlog::debug("    View [{}]: Recommended Width={} Height={} SampleCount={}", i, view.recommendedImageRectWidth,
                      view.recommendedImageRectHeight, view.recommendedSwapchainSampleCount);
        spdlog::debug("    View [{}]:     Maximum Width={} Height={} SampleCount={}", i, view.maxImageRectWidth,
    ...
    CHECK_XRCMD(
        xrEnumerateEnvironmentBlendModes(instance, system_id, view_config_type, count, &count, blend_modes.data()));
...
void LogActionSourceName(XrSession session, XrAction action, const std::string &action_name) {
  ...
  CHECK_XRCMD(xrEnumerateBoundSourcesForAction(session, &get_info, static_cast<uint32_t>(paths.size()), &path_count,
                                               paths.data()));
  ...
  for (uint32_t i = 0; i < path_count; ++i) {
    ...
    CHECK_XRCMD(xrGetInputSourceLocalizedName(session, &name_info, static_cast<uint32_t>(grab_source.size()), &size,
                                              grab_source.data()));
