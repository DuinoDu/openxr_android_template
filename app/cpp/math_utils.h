...
inline static glm::mat4 CreateProjectionFov(const XrFovf fov, const float near, const float far) {
...
inline static glm::mat4 InvertRigidBody(const glm::mat4 src) {
  ...
  result_ptr[11] = 0.0f;
  result_ptr[12] = -(src_ptr[0] * src_ptr[12] + src_ptr[1] * src_ptr[13] + src_ptr[2] * src_ptr[14]);
  result_ptr[13] = -(src_ptr[4] * src_ptr[12] + src_ptr[5] * src_ptr[13] + src_ptr[6] * src_ptr[14]);
  result_ptr[14] = -(src_ptr[8] * src_ptr[12] + src_ptr[9] * src_ptr[13] + src_ptr[10] * src_ptr[14]);
  result_ptr[15] = 1.0f;
  return result;
}
inline static glm::vec3 XrVector3FToGlm(XrVector3f xr_vector) { return {xr_vector.x, xr_vector.y, xr_vector.z}; }
