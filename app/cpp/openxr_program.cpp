...
void OpenXrProgram::CreateInstance() {
  ...
  std::transform(kPlatformExtensions.begin(), kPlatformExtensions.end(), std::back_inserter(extensions),
                 [](const std::string &ext) { return ext.c_str(); });

  const std::vector<std::string> kGraphicsExtensions = graphics_plugin_->GetOpenXrInstanceExtensions();
  std::transform(kGraphicsExtensions.begin(), kGraphicsExtensions.end(), std::back_inserter(extensions),
                 [](const std::string &ext) { return ext.c_str(); });
...
void OpenXrProgram::InitializeActions() {
  ...
  CHECK_XRCMD(xrStringToPath(instance_, "/user/hand/right/input/trigger/value", &trigger_value_path[side::RIGHT]));
  {
    ...
    CHECK_XRCMD(xrStringToPath(instance_, "/interaction_profiles/oculus/touch_controller",
                               &oculus_touch_interaction_profile_path));
    std::vector<XrActionSuggestedBinding> bindings{{{input_.grab_action, squeeze_value_path[side::LEFT]},
...
void OpenXrProgram::CreateSwapchains() {
  ...
  CHECK_XRCMD(xrEnumerateSwapchainFormats(session_, static_cast<uint32_t>(swapchain_formats.size()),
  ...
  CHECK_XRCMD(xrEnumerateViewConfigurationViews(instance_, system_id_, view_config_type_, view_count, &view_count,
                                                config_views_.data()));
  ...
  for (const auto &view_config_view : config_views_) {
    spdlog::info("Creating swapchain with dimensions Width={} Height={} SampleCount={}",
                 view_config_view.recommendedImageRectWidth, view_config_view.recommendedImageRectHeight,
  ...
  while (const XrEventDataBaseHeader *event = TryReadNextEvent()) {
    switch (event->type) {
      case XR_TYPE_EVENT_DATA_INSTANCE_LOSS_PENDING: {
        const auto &instance_loss_pending = *reinterpret_cast<const XrEventDataInstanceLossPending *>(&event);
        spdlog::warn("XrEventDataInstanceLossPending by {}", instance_loss_pending.lossTime);
        return;
      }
      case XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED: {
        auto session_state_changed_event = *reinterpret_cast<const XrEventDataSessionStateChanged *>(event);
...
void OpenXrProgram::HandleSessionStateChangedEvent(const XrEventDataSessionStateChanged &state_changed_event) {
  spdlog::info("XrEventDataSessionStateChanged: state {}->{} time={}", session_state_, state_changed_event.state,
...
bool OpenXrProgram::IsSessionRunning() const { return session_running_; }
...
bool OpenXrProgram::RenderLayer(XrTime predicted_display_time,
  ...
  CHECK_XRCMD(
      xrLocateViews(session_, &view_locate_info, &view_state, views_.size(), &view_count_output, views_.data()));
  ...
  for (XrSpace visualized_space : visualized_spaces_) {
    ...
    if (XR_UNQUALIFIED_SUCCESS(res)) {
      if ((space_location.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0 &&
        ...
        cubes.push_back(math::Transform{math::XrQuaternionFToGlm(space_location.pose.orientation),
  ...
  // Render a 10cm cube scaled by grab_action for each hand. Note renderHand will only be true when the application has
  // focus.
  for (auto hand : {side::LEFT, side::RIGHT}) {
    XrSpaceLocation space_location{};
    space_location.type = XR_TYPE_SPACE_LOCATION;
  ...
  for (uint32_t i = 0; i < view_count_output; i++) {
    ...
    CHECK_XRCMD(xrAcquireSwapchainImage(view_swapchain.handle, &acquire_info, &swapchain_image_index));
    ...
    projection_layer_views[i].subImage.imageRect.extent = {view_swapchain.width, view_swapchain.height};

    auto swapchain_image = swapchain_images_[view_swapchain.handle];
    graphics_plugin_->RenderView(projection_layer_views[i], swapchain_image, swapchain_image_index, cubes);
